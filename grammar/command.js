...
module.exports = {
  ...
  _decl_sig: $ => seq(
    alias(
      repeat(choice($._simple_binder_without_type, $._bracketed_binder)),
      $.binders,
    ),
    $._type_spec,
  ),
  _opt_decl_sig: $ => min1(
    alias(
      repeat1(choice($._simple_binder_without_type, $._bracketed_binder)),
      $.binders,
    ),
    $._type_spec,
  ),
  _decl_val_simple: $ => seq(':=', $._term, optional($._where_decls)),
  ...
  _extends: $ => field('extends', seq('extends', sep1($._term, ','))),
  structure: $ => seq(
